...
public class EmailMessage {
	...
	public String[] ccRecipientsArray = new String[] { };
	/* Target MX-host */
	...
	public EmailMessage(String from, String to, String cc, String subject, String text,
		...
		Sender = from.trim();
		Recipient = to.trim();
		ccRecipients = cc.trim();
		Subject = subject.trim();
		
		Headers = "From: " + Sender + CRLF;
		Headers += "To: " + Recipient + CRLF;
		Headers += "CC: " + ccRecipients + CRLF;
		Headers += "Subject: " + Subject + CRLF;
		
		/* Split CC field text into an array of recipient addresses */
		...
		for(int i = 0; i < arrayLength; i = i + 1){
			...
			if ((ccAddress.length()) <= 1) {
				...
				if(ccAddress.indexOf('@') <1){
					System.out.println("CC recipient:" + ccAddress + " is not valid");
					return false;
				}
			};
		}
